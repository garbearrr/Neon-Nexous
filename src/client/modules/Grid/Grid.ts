...
export class Grid {
    ...
    private Connections: Collection<string, RBXScriptConnection> = Collection();
    private Drag: CFrame[] = [];
    private DragLockedHorizontal: boolean = false;
    private DragLockedVertical: boolean = false;
    private DragStart?: Vector2;
    private FireSignals: Collection<string, FireSignal> = Collection();
    ...
    public static Instance(GridParent?: Part): Grid {
        if (Grid._instance !== undefined) return Grid._instance;
        if (GridParent === undefined && Grid.ParentGridPart === undefined) {
            return error("ParentGridPart must be provided to create a Grid instance.");
        }

        Grid.ParentGridPart = GridParent || Grid.ParentGridPart;
        return Grid._instance = new Grid();
    }
    ...
    private GetItemSizeInCellsXY(): Vector2 {
        const ItemSize = this.ItemSize;
        const shouldflip = Grid.Rotation / 90 === 1 || Grid.Rotation / 90 === 3;
    ...
    private OnRayHit(_c: ActiveCast, Res: RaycastResult, _v: Vector3, _b: Instance) {
        ...
        const TargetOrientation = new Vector3(0, math.rad(Grid.Rotation), 0);
